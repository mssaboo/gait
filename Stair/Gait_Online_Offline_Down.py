...
while (a<limit):
    ...
    Max=0
    ToeOff=0
    if((cp0<cp5)and(SANX[a]<-25)and(abs(SANX[a])<180)):
        cp0=a
    if((cp1<cp0)and(SANX[a]>-3)and(abs(SANX[a])<180)):
        cp1=a
    if((cp2<cp1)and(abs(SACX[a])>3)):
        cp2=a
        Strike=1
        TS.append(cp2)
        flag1=1
    ...
    if((flag1==1)and(AAVY_count==5)):
        ...
        flag1=0
        cp3=a
    if((cp4<cp3)and(AAVY[a]>60)and(AAVY[a]>AAVY[a-1])and(abs(AAVY[a])<500)and(abs(AAVY[a-1])<500)):
        cp4=a
        Max=1
        TM.append(cp4)
    if((cp5<cp4)and((AAVY[a]>220)or(AAVY[a]<AAVY[a-1]))and(abs(AAVY[a])<500)and(abs(AAVY[a-1])<500)and(AAVY[a]>120)):
        cp5=a
        ToeOff=1
        TO.append(cp5)
