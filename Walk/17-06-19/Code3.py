...
def HM_fun1 (data,count=0):
	...
	while(flag!=1):
		if((data.AANY[count+1]>data.AANY[count])):
			flag=1
		else:
			count=count+1
		if(count>=limit):
...
def HM_fun2 (data,count=0):
	flag=0
	limit_HM = count+(limit/150)
	while(flag!=1):
		if(abs(data.SAVZ[count])<52):
			flag=1
		else:
			count=count+1
		if(count>=limit):
...
def HO_fun4 (data,count=0):
	flag=0
	limit_HO=count+(limit/100)
	while(flag!=1):
		if(data.SAVZ[count]>41):
			flag=1
		else:
			count=count+1
		if(count>=limit):
...
while(count<=limit):
	...
	#count = HM_fun2(data,count)
	#HM.append(count)
	#count = HO_fun4(data,count)
	#HO.append(count)
	#count = count+1
